app = {
	...
	wxUserInfo:function(success){
		...
		var ticketInfoId = app.getQueryString('ticketInfoId');
		if(enterpriseCode && ticketGroupNum && ticketInfoId){
			app.setCookie('enterpriseCode', enterpriseCode);
			app.setCookie('ticketGroupNum', ticketGroupNum);
			app.setCookie('ticketInfoId', ticketInfoId);
		}
	...
	getTicketInfo: function(){
		app.getAjax('orderEquity/verify', {orderId: app.getCookie('ticketInfoId')}, function(res){
			if(res.success){
				
			}else{
				$('.payBtn').text('查看订单');
				$('.payBtn').attr('onclick', 'orderClick()');
				layer.alert(res.message);
			}
		})
	},
