app = {
	...
	wxUserInfo:function(success){
		...
		if(this.getQueryString('openid') == '' || this.getQueryString('openid') == null || this.getQueryString('openid') == 'null') {
			$.ajax({
				...
				if(ybyNp60){
					app.setCookie('ybyNp60', ybyNp60);
				}else{
					app.clearCookie('ybyNp60');
				}
				if(ybyNp120){
					app.setCookie('ybyNp120', ybyNp120);
				}else{
					app.clearCookie('ybyNp120');
				}
				console.log(app.getCookie('ybyNp120'));
	...
	encryptGetAjax:function(u,params,callback){
		$.ajax({
	        url:this.root + u,
	        type:"GET",
	...
	encryptPostAjax:function(u,params,callback){
		$.ajax({
	        url:this.root + u,
	        type:"POST",
			headers: createHeaders(params),
