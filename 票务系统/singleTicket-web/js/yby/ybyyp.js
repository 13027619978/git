	...
	app.getAjax('ticketInfo/get', {
		...
		if(res.code == '10000'){
			...
			visitorInfo = '<div class="visitorItem">' +
				...
				'<div class="itemInfo">' +
					'<p>企业名称：</p>' +
					'<input class="companyName" type="text" placeholder="请输入企业名称">' +
				'</div>' + 
				'<div class="itemInfo">' +
					'<p>企业部门：</p>' +
					'<input class="remark" type="text" placeholder="请输入企业部门">' +
				'</div>' + 
			'</div>';
...
function payClick(){
	...
	for(var i = 0; i < ticketNumber; i++){
		...
		var companyName = $('.companyName').eq(i).val();
		var remark = $('.remark').eq(i).val();

		if(idCardRequired == '1'){
			if(!idCard || !name || !phone || !companyName || !remark){
			...
			if(!name || !phone || !companyName || !remark){
				layer.alert('请先完善游客信息');
				// 启用支付按钮
				$('.payBtn').attr('onclick', 'payClick()');
				return;
	...
	for(var i = 0; i < ticketNumber; i++){
		...
		visitor = {
			...
			companyName: $('.companyName').eq(i).val(),
			remark: $('.remark').eq(i).val()
		}
