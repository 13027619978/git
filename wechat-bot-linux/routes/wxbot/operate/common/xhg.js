...
function getXcCheckInfo(room){
	...
	let totalNumber = 0;
	let totalMoney = 0;
	axios.get('https://boss.smart-ideas.com.cn/ticketApi/robotCollection/check/name/get?enterpriseCode=TgsEpcXhg&ticketGroupNum=TGN20211210160317254&checkStartTime='+ searchSdate +'&checkEndTime=' + searchEdate)
		.then(function(res){
			res.data.data.forEach(function(value, key){
				if(value.name == '雪场双人套票'){
					xctp = value.checkQuantity;
					xctpMoney = value.checkMoney;
					totalNumber += parseInt(value.checkQuantity);
					totalMoney += parseInt(value.checkMoney);
				}
				
				if(value.name == '雪场门票'){
					xc = value.checkQuantity;
					xcMoney = value.checkMoney;
					totalNumber += parseInt(value.checkQuantity);
					totalMoney += parseInt(value.checkMoney);
				}
			})
			botString += '总核销：' + totalNumber +'\n';
			botString += '总核销金额：' + totalMoney + '\n';
			botString += '--------------------\n';
			botString += '雪场门票：' + xc + '张 ' + xcMoney + '元\n';
			botString += '雪场双人套票：' + xctp + '张 ' + xctpMoney  +'元';
			room.say(botString);
		})
		.catch(function(err){
...
function getXcCheckPeopleInfo(room){
	var nowDate = new Date();
	var year = nowDate.getFullYear();
	var month = nowDate.getMonth()+1;
	month = month>9?month:'0'+month;
