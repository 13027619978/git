...
async function xwhzjcRoomDeal(msg) {
	...
	if (msg.type() == wechat.bot.Message.Type.Text) {
		...
		if (content.indexOf('船号') != -1 && content.indexOf('手机号') != -1 && content.indexOf('现金开锁') != -1) {
			...
			if (isNumber(code)) {
				const options = {
					hostname: host,
					path: '/xwhpark/deviceLeaseApp/getDeviceInfo?openid=cash&deviceSn='+ code,
					method: 'GET'
				};
				...
				const req = http.request(options, (res) => {
				  res.on('data', (d) => {
					...
					if(res.code == 'SUCCESS'){
						...
						const options1 = {
							hostname: host,
							path: '/xwhpark/deviceLeaseApp/getPayInfo?openid=cash&deviceId='+ deviceId + '&phone=' + phone + '&deposit=' + deposit,
							method: 'GET'
						};
...
function completeRescue(host, room, code, contact) {
	const options = {
		hostname: host,
		path: '/xwhpark/deviceRobot/removeRescue?deviceSn=' + code,
		method: 'GET'
	};
		...
		res.on('data', (d) => {
			...
			if(type == 'open'){
				...
				fs.readFile(path.resolve(__dirname, '../jsonData/xwhOpen.json'), 'utf8', function(err, data){
					...
					var tldxOpen = parseInt(data.tldx);
					if(res.data == xwhRoomWharfId.JFM){
						...
						hzOpen += 1;
					}else if(res.data == xwhRoomWharfId.TLDX){
						tldxOpen += 1;
					}
					var xwhData = {
						"jfm": jfmOpen,
...
function updateStatus(status, host, room, code, contact) {
	const options = {
		hostname: host,
		path: '/xwhpark/deviceRobot/updateStatus?deviceSn=' + code + '&status=' + status,
		method: 'GET'
	};
...
function getLastedPhone(host, room, code, contact) {
	const options = {
		hostname: host,
		path: '/xwhpark/deviceRefundRobot/getLastedPhone?deviceSn=' + code,
		method: 'GET'
	};
...
function refund(host, room, refundType, refundValue, refundWall, refundMoney, contact) {
	...
	const options = {
		hostname: host,
		path: '/xwhpark/deviceRefundRobot/refund',
		method: 'POST',
		headers: {
