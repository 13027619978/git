...
/***** 圆明园自驾船群代码 *******/
/********************/
async function ymyzjcRoomDeal(msg){
...
function completeRescue(host, room, code, contact){
	const options = {
		hostname: host,
		path: '/ymypark/deviceRobot/removeRescue?deviceSn='+ code,
		method: 'GET'
	};
...
function updateStatus(status, host, room, code, contact){
	const options = {
		hostname: host,
		path: '/ymypark/deviceRobot/updateStatus?deviceSn='+ code +'&status=' + status,
		method: 'GET'
	};
...
function getLastedPhone(host, room, code, contact){
	const options = {
		hostname: host,
		path: '/ymypark/deviceRefundRobot/getLastedPhone?deviceSn='+ code,
		method: 'GET'
	};
