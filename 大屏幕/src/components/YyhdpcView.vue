    ...
    <dv-full-screen-container class="container">
      ...
      <div class="contentView">
        ...
        <div class="leftView">
          <div class="leftContent">
            ...
            <div class="rightNotice">
              ...
              <dv-scroll-board :config="refundConfig" style="width:100%;height:180px" />
              <!-- 租赁详情 -->
              <div class="leaseDetailView">
                <p class="noticeTitle">租赁详情</p>
                <dv-scroll-board :config="leaseConfig" style="width:100%;height:120px" />
              </div>
            </div>
        ...
        <div class="centerView">
          ...
          <div class="mapView">
            <dv-border-box-7>
              <div class="iframeView">
                <iframe src="http://smart-ideas.com.cn/mapView/yyhmap.html" frameborder="0" width="680px" height="750px"></iframe>
              </div>
            </dv-border-box-7>
          </div>
          ...
          <!-- 温馨提示 -->
          <div class="wxts">
            <dv-border-box-10>温馨提示：欢迎乘坐野鸭湖游船</dv-border-box-10>
          </div>
        </div>
        ...
        <div class="rightView">
          ...
          <!-- 价格须知 -->
          <div class="priceView">
            <p class="noticeTitle">租赁规则</p>
            <p>四人船160元/小时，六人船200元/小时，押金500元。</p>
            <p>一小时起租，超过一小时后按半小时累加。</p>
            <p>救援电话：13021967698</p>
          </div>
...
export default {
  ...
  methods: {
    createCode(scenicId){
      let qrcode = new QRCode('qrcode', {
        width: 150,  // 二维码宽度
        height: 150, // 二维码高度
        text: 'http://lease.smart-ideas.com.cn/yyhWeb/yyhPosRefund/?type=boat'
      })
    },
    clearCode () {
  ...
  mounted: function(){
    let scenicId = 'bcd98681896f48ed878486519bfdeacf';
    let that = this;
    // 初始化时间
    ...
    setInterval(function(){
      ...
      if(seconds == 1 || seconds % 20 == 0){
        // 获取租赁详情
        axios.get('http://lease.smart-ideas.com.cn/yyhpark/deviceDataView/getDeviceShipLeaseInfo')
          .then(function(res){
        ...
        // 获取退款详情
        axios.get('http://lease.smart-ideas.com.cn/yyhpark/deviceDataView/getDeviceShipRefundInfo')
          .then(function(res){
        ...
        // 获取在租人数
        axios.get('http://lease.smart-ideas.com.cn/yyhpark/deviceDataView/getDeviceShipLeasePercent?type=6')
          .then(function(res){
            console.log(res);
            var fourBoat = parseInt(res.data[0].y);
            axios.get('http://lease.smart-ideas.com.cn/yyhpark/deviceDataView/getDeviceShipLeasePercent?type=7')
              .then(function(res){
                console.log(res);
                var totalBoatOnlease = fourBoat + parseInt(res.data[0].y);
                console.log(totalBoatOnlease);
                that.config1 = {
                  number: [parseInt(totalBoatOnlease)],
                  content: '{nt}',
                  style: {
                    fontSize: 120,
                    fill: '#0aff4d',
