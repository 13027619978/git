                ...
                <dv-border-box-10>
                  <div class="zdIncome">
                    <div class="zdIncomeItem">
                      ...
                      <dv-charts :option="incomeData" style="width:627px;height:230px"/>
                    </div>
                    <div class="zdIncomeItem">
                      <p class="title">周统计</p>
                      <p class="totalIncome">总收入：{{num==0?dayIncome:num==1?weekIncome:monthIncome}}元</p>
                      <dv-charts :option="incomeData1" style="width:627px;height:230px"/>
                    </div>
                    <div class="zdIncomeItem">
                      <p class="title">月统计</p>
                      <p class="totalIncome">总收入：{{num==0?dayIncome:num==1?weekIncome:monthIncome}}元</p>
                      <dv-charts :option="incomeData2" style="width:627px;height:230px"/>
                    </div>
                  </div>
                </dv-border-box-10>
              </div>
...
export default {
  ...
  methods: {
    ...
    zdTab: function(num){
      let that = this;
      // 站点售卡数统计
      axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getSiteSaleInfo?lineId=' + num)
        .then(function(res){
          var dataList = [];
          var restDataList = [];
          var nameList = [];
  ...
  mounted: function(){
    ...
    setInterval(function(){
      ...
      if(seconds == 1 || seconds % 20 == 0){
          ...
          // 获取消息推送
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getOrderInfo')
            .then(function(res){
              var dataList = [];
              res.data.forEach(function(value, key){
                var siteName = value.siteName;
          ...
          // 预警
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/batteryDataView/getPrinterStatus')
            .then(function(res){
              var dataList = [];
              res.data.forEach(function(value, key){
                if(value.status != '0'){
          ...
          // 收入曲线
          let type;
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getIncome?lineId=1')
            .then(function(res){
              var nhxDayDataList = [];
              var nhxDayTimeList = [];
              var nhxWeekDataList = [];
          ...
          // 获取卡尺位置
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getVehicleMountedLocation?lineId=1')
            .then(function(res){
              var carList = [];
              var snList = [];
              var idList = [];
          ...
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getVehicleMountedLocation?lineId=2')
            .then(function(res){
              var carList = [];
              var snList = [];
              var idList = [];
          ...
          // 获取照片
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/loopDataV/getVehicleMountedInfo')
            .then(function(res){
              console.log(res);
          ...
          // 获取收入比例
          axios.get('http://rent.smart-ideas.com.cn/nbhpark/operationDataView/getCurrentCollection')
            .then(function(res){
