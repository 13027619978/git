      ...
      success: function (res) {
        ...
        if (res.result.indexOf("code=") != -1) {
          ...
          if (res.result == app.globalData.refundUrl) {
            ...
            let qrType = res.result.split('qrType=')[1];
            if (qrType == 'batteryCar' && res.result.split('?')[0] + '/' == app.globalData.refundUrl) {
              wx.navigateTo({
                url: '../refund/refund?qrType=batteryCar&url=' + res.result,
              })
            }
          }
        }
  ...
  onLoad: function (options) {
    ...
    wx.login({
      success: function (res) {
        console.log(res);
        wx.request({
          url: app.globalData.url + 'wx/miniOpenId?code=' + res.code,
          method: 'get',
          success: function (res) {
            wx.hideLoading();
            wx.setStorage({
            ...
            wx.request({
              url: app.globalData.url + 'wxUserApp/verifyWxUser',
              method: 'get',
              data: {
