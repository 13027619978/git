...
Page({
   ...
   **/
  data: {
    longitude: 116.692490,
    latitude: 40.180465,
    scale: 17,
    distanceHidden: true,
    // categoryList: ['景点', '自行车服务', '电瓶车服务', '游船服务', '环线观光车服务'],
    categoryList: ['景点', '自行车服务', '电瓶车服务', '游船服务'],
    currentIndex: 0,
    ...
    wx.getLocation({
      ...
      success: function(res) {
        ...
        if(that.data.currentIndex == 0){
          switch (e.markerId) {
            case "0":
              markerInfo.name = '未堂小镇';
              break;
          }
        }else if(that.data.currentIndex == 1){
          markerInfo.name = '自行车租赁处';
        } else if (that.data.currentIndex == 2) {
          markerInfo.name = '电瓶车租赁处';
            ...
            success: function (res) {
              if (res.confirm) {
                wx.openLocation({
                  ...
                  name: '北京奥林匹克水上公园-' + markerInfo.name
                })
              }
            }
          });
