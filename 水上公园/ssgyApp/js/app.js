app = {
	root: 'https://iot.smart-ideas.com.cn/ssgy/',
	
	
	//商卖获取用户信息
	payUserInfo:function(){
		var indexhref;
		var payType = this.getQueryString('payType');
		if(payType){
		...
		if(this.getQueryString('openid') == '' || this.getQueryString('openid') == null || this.getQueryString('openid') == 'null') {
			window.location.href = this.root+'wx/snsapiuser?url='+indexhref;
			return;
		}
	},
	...
	wxUserInfo:function(){
		...
		if(this.getQueryString('openid')!=null && this.getQueryString('openid')!=undefined){
		 	this.setCookie('openid',this.getQueryString('openid'),{expires:24*7});
		 	$(".avatar_img").attr('src',this.getQueryString('headimgurl'));
		}
		if(this.getQueryString('openid') == '' || this.getQueryString('openid') == null || this.getQueryString('openid') == 'null') {
			window.location.href = this.root+'wx/snsapiuser?url='+indexhref;
			return;
